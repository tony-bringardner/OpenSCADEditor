...
%{
	 ...
	 */
	public final void yyresetTonyB(java.io.Reader reader) {
		// 's' has been updated.
		zzBuffer = s.array;
		/*
...
<YYINITIAL> {
	...
	"render" |
	"children" |
	"intersection" 		{ addToken(Token.RESERVED_WORD); }
	
	
	// 2D & 3D  objects
	"circle"						{ addToken(Token.DATA_TYPE); }
	"square"						{ addToken(Token.DATA_TYPE); }
	"polygon"						{ addToken(Token.DATA_TYPE); }
	"text"						{ addToken(Token.DATA_TYPE); }
	"sphere"						{ addToken(Token.DATA_TYPE); }
	"cube"						{ addToken(Token.DATA_TYPE); }
	"cylinder"						{ addToken(Token.DATA_TYPE); }
	"polyhedron"						{ addToken(Token.DATA_TYPE); }
	"undef"						{ addToken(Token.RESERVED_WORD); }
